#!/usr/bin/env python
# Name:     neo.py
# Purpose:  Check IP Address for Malicious Indicators
# By:       Armand Kruger
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--ip', help='IP address to Match')
    ...
    if args is not None and args.ip is not None and len(args.ip) > 0:
        ...
        print(green('Check IP For Malicious Indicators'))
        print(green('By : Armand Kruger\n'))
        print(red('Your public IP address is {0}\n'.format(my_ip)))

        # Get IP 
        resp = raw_input('Check {0} ? (Y/N):'.format(my_ip))

        if resp.lower() in ["yes", "y"]:
            badip = my_ip
        else:
            badip = raw_input(green("\nIP To Investigate: "))
            if badip is None or badip == "":
                sys.exit("No IP address Found")
